...
工作区的知识目录（agentspace 为根 `knowledge/`）是 OKF v0.2 bundle，仅保存稳定、可复用、脱离当前会话仍可理解的知识。

## 文件角色

- bundle 根 `index.md`（agentspace：`knowledge/index.md`）声明 `okf_version: "0.2"`，并作为唯一入口索引。
- bundle 根 `log.md` 只记录重要且已确认的知识变更。
